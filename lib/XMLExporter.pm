...
sub fetchSpaces {
    ...
    $spaces = $confluence->call("$Confluence::API.getSpaces",
                                         $token)->result();
...
sub extractSiteXML {
    ...
    $logger->info("NOTE: Check Exported_URLS file for list of extracted spaces");
...
sub extractSpaceXML {
    ...
    while ( my ($spacekey,$spacename) = each %spaces ) {
	    ...
	    my $url = ($confluence->call("confluence1.exportSpace",
		        $token, $spacekey,"TYPE_XML")->result());
